...
func (m *Mesh) DoSort() {
	...
	// Now set the indices
	ndxprev := m.Ndx[0]
	m.Grid[ndxprev] = &GridPoint{N: ndxprev, I: m.flat2three(0), Lo: 0}
	for ii, ndx1 := range m.Ndx {
		if ndx1 != ndxprev {
			m.Grid[ndxprev].Hi = ii
			m.Grid[ndx1] = &GridPoint{N: ndx1, I: m.flat2three(ndx1), Lo: ii}
			ndxprev = ndx1
		}
	}
	m.Grid[ndxprev].Hi = m.Npart
}
